...
def read_miles(excel_path):
    ...
    with open('miles_s_m_h_04_20.json', 'w') as json_file:
...
if __name__ == "__main__":
    ...
    hours = 8
    # num_vehicles = collect_vehicle(ditribution_json_path04, hours)
    ...
    for minutes, miles_per_minutes in ml_04[str(hours)].items():
        print("minutes: ", minutes, "active_vehicle: ", len(new_vehicle_database.dict_active_vehicle))
        for seconds, miles_per_seconds in miles_per_minutes.items():
            lst_txn = generate_txn(hours, minutes, seconds, miles_per_seconds, new_vehicle_database)
            current_time = (hours * 3600) + (int(minutes) * 60)
    ...
    dict_confirmed_txn_statistics = txn_database.confirmed_txn_stastic

    json_str = json.dumps(dict_confirmed_txn_statistics, sort_keys=True, indent=4)
